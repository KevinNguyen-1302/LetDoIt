...
export const CategoryIcon = ({
  ...
  const IconComponent =
    (Icons as any)[formattedName] || (Icons as any)[iconName] || Icons.Folder;
...
const TaskCardDetail = ({ task, isOpen, onClose, columns }: Props) => {
  ...
  useEffect(() => {
    if (task) {
      ...
      const localTime = isUtc
        ? taskDateObj.toLocaleTimeString("sv-SE", {
            hour: "2-digit",
            minute: "2-digit",
          }) // HH:MM
        : "";
  ...
  const getPriorityBadgeColor = (priority: number) => {
    switch (priority) {
      ...
      case 3:
        return "bg-orange-100 text-orange-950 border-black";
      case 4:
        return "bg-red-100 text-red-900 border-black";
      default:
        return "bg-gray-100 text-gray-900 border-black";
    }
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-3.5 right-4 text-black hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none transition-transform active:scale-95 border-2 border-black bg-white rounded-full p-1.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
        >
        ...
        {isEditing ? (
          ...
          <form onSubmit={handleSave} className="space-y-4 pt-2">
            <h3 className="font-cherry text-3xl mb-4 text-gray-900">
              Edit Task
            </h3>
          ...
          <div className="space-y-6 pt-2">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              {/* Priority badge */}
              <span
                className={`text-xs px-3 py-1 font-black rounded-full border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] uppercase tracking-wider ${getPriorityBadgeColor(task.priority)}`}
              >
              ...
              {/* Status Badge */}
              <span
                className={`text-xs px-3 py-1 font-bold rounded-full border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] flex items-center gap-1.5 ${task.isCompleted ? "bg-[#548e76] text-white" : "bg-white"}`}
              >
                {task.isCompleted ? (
                  <CheckCircle size={14} />
                ) : (
                ...
                {task.isCompleted
                  ? "Completed"
                  : columns.find((c) => c.columnId === task.columnId)?.title}
              </span>
              ...
              <span className="text-xs px-3 py-1 font-bold rounded-full border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] flex items-center gap-1.5">
                <Apple size={14} />
                {task.createdByName || "Unknown"} ( Creator )
              </span>
            </div>
              ...
              {task.assigneeName ? (
                <div className="flex items-center gap-2 pl-1 pr-2 py-1 mt-5 min-w-20 max-w-46 rounded-full border-2 border-black bg-[#E8FF46]">
                  {task.assigneeAvatarUrl ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-[#E8FF46] border border-black flex items-center justify-center text-[12px] font-black uppercase text-black">
                ...
                <div className="flex items-center gap-1 text-gray-500 font-semibold italic">
                  <Icons.User size={16} className="opacity-60" />
            ...
            <div className="bg-white/50 border-2 border-black rounded-2xl p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] min-h-25 flex flex-col justify-between">
              <div>
                <h4 className="text-xs font-black uppercase text-gray-600 tracking-wider mb-2">
                  Description
                </h4>
