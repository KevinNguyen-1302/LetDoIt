...
import { updateProject, changeProjectAuthor, getProjectMembers } from "../services/projectService";
  ...
  const handleSave = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (editAuthor !== authorName && editAuthor.trim()) {
        try {
          const selectedMember = members.find((m) => m.username === editAuthor.trim());
  ...
  return (
    <div
      className={`group relative bg-white border-2 border-black rounded-xl p-5 transition-all duration-300 ease-in-out flex flex-col justify-between cursor-pointer ${
        editMode
          ? "scale-105 z-10 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] min-h-[200px]"
          : "hover:-translate-y-1 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] min-h-[160px]"
      }`}
    >
      {/* Top Header */}
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                {isLoadingMembers ? (
                  ...
                  <select
                    ...
                    disabled={isLoading}
                  >
                    {!members.find((m) => m.username === authorName) && authorName && (
