...
const Register = () => {
  ...
  const RuleItem = ({
    ...
    <div
      className={`flex items-center text-xs mt-1 transition-colors duration-300 ${isPassed ? "text-green-500" : "text-gray-400"}`}
    >
      <span className="mr-2">{isPassed ? "●" : "○"}</span>
      {text}
    </div>
  );
  ...
  const handleSubmit = async (e: SyntheticEvent) => {
    ...
    if (step === 2) {
      ...
      if (!displayname || !dob) {
        console.warn(
          "❌ Missing required fields - displayname:",
          displayname,
          "dob:",
          dob,
        );
      ...
      try {
        console.log(
          "🚀 Sending register request to http://localhost:5112/api/user/register",
        );
            ...
            {step === 1 && (
              <div className="grid grid-cols-1">
                ...
                <div className=" p-3 mb-4 bg-gray-50 rounded-md border border-gray-100">
                  ...
                  <RuleItem
                    isPassed={rules.uppercase}
                    text="Ít nhất 1 chữ in hoa"
                  />
                  <RuleItem
                    isPassed={rules.lowercase}
                    text="Ít nhất 1 chữ thường"
                  />
                  <RuleItem
                    isPassed={rules.specialChar}
